...
export default function PasswordScreen({ onUnlock }: PasswordScreenProps) {
  ...
  useEffect(() => {
    // Generate floating hearts particles on the login screen
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (cleanPassword === '202917' || cleanPassword === '525252' || cleanPassword === '2029') {
      setIsSuccess(true);
      setError('');
      setTimeout(() => {
        onUnlock(cleanPassword);
      }, 1200);
    } else {
        ...
        {!isSuccess ? (
          ...
          >
            ...
            <div className="mt-6 mb-8">
              <h1 className="text-3xl font-bold font-serif text-rose-500 mb-2 mt-2 glow-romantic">
                Наш уютный уголок 🤍
              </h1>
              <p className="text-gray-600 text-sm px-4">
                Привет, моя любимая девочка! Я создал этот особенный сюрприз для нас. Чтобы войти, веди нужный код
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="w-full py-4 bg-gradient-to-r from-rose-400 to-pink-500 hover:from-rose-500 hover:to-pink-600 text-white font-medium rounded-2xl shadow-md hover:shadow-lg active:scale-95 transition-all duration-200 flex items-center justify-center gap-2 group"
          ...
          <motion.div
            ...
            className="relative z-10 text-center text-rose-600 space-y-4"
          >
            <motion.div
              animate={{ rotate: [0, 15, -15, 0], scale: [1, 1.2, 1] }}
              transition={{ repeat: Infinity, duration: 1.5 }}
              className="inline-block bg-white text-rose-500 p-6 rounded-full shadow-2xl border-4 border-rose-400"
            >
            ...
            <motion.h2
              initial={{ y: 20 }}
              animate={{ y: 0 }}
              className="text-4xl font-bold font-serif text-rose-600 glow-romantic"
            >
              Пароль верный! 🎉
            </motion.h2>
            <p className="text-rose-500/80 font-serif text-lg tracking-wide">
              Добро пожаловать в нашу историю любви... Открываю занавес!
            </p>
          </motion.div>
        )}
      </AnimatePresence>
