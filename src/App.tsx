  ...
  useEffect(() => {
    const syncPhotos = async () => {
      try {
        ...
        if (loaded.length > 0) {
          // Sort photos to show latest added first (by reverse natural alphanumeric ID sorting or ID value)
          loaded.sort((a, b) => b.id.localeCompare(a.id));
          setPhotos(loaded);
        } else {
          // If Firestore is completely fresh, seed the initial photos into the DB so they are forever backed up
